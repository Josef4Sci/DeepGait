...
class DecoderSin(nn.Module):
    ...
    def forward(self, latent):
        ...
        out = torch.sum(sin, dim=1)+D.view(self.batch_size, 1, self.output_size).expand(self.batch_size, self.sequence_length, self.output_size)
...
class SpeedNET(nn.Module):
    ...
    def fit(self, num_epochs, train_dataset, test_dataset, valid_dataset= None, shuffleTrain=True):
        ...
        # while self.samplesIncluded<200:
            ...
            for (samples, labels, weight, height) in tqdm(train_data_loader):
                ...
                loss, loss_dummy, loss_vrae, latent, loss_sp = self.filter_step(samples, labels, weight, height, self.device,
                                                                       optimizer, MSE)
                loss_x += loss.item()
            ...
            if valid_dataset:
                ...
                for i, (samples, labels, weight, height) in enumerate(valid_data_loader):
                    with torch.no_grad():
                        loss_speed, loss_dummy, loss_vrae, latent, loss_sp = self.filter_step(samples, labels, weight, height,
            ...
            if test_dataset:
                ...
                for i, (samples, labels, weight, height) in enumerate(test_data_loader):
                    with torch.no_grad():
                        loss_MSE, loss_MAE, loss_vrae, latent, loss_sp = self.filter_step(samples, labels, weight, height,
    ...
    def filter_step(self, samples, labels, trainW, trainH, device, optimizer, loss1, loss2=None, train=True):
        ...
        loss1res = loss_speed * DFF_multip + recon_loss * encod_decod_multip + kl_loss #+ self.mult*loss_aten
