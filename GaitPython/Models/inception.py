...
class Inception(nn.Module):
	...
	def forward(self, X):
		...
		if self.return_indices:
			return Z, Z_bottleneck, indices
		else:
			return Z, Z_bottleneck
...
class InceptionBlock(nn.Module):
	def __init__(self, in_channels, n_filters=32, kernel_sizes=[9,19,39], bottleneck_channels=32, use_residual=True, activation=nn.ReLU(), return_indices=False):
		...
		self.conv_from_bottleneck_decoder = nn.Conv1d(
									in_channels=bottleneck_channels,
	...
	def forward(self, X):
		if self.return_indices:
			Z, bottleneck1, i1 = self.inception_1(X)
			Z, bottleneck2, i2 = self.inception_2(Z)
			Z, bottleneck3, i3 = self.inception_3(Z)
		else:
			Z, bottleneck1 = self.inception_1(X)
			Z, bottleneck2 = self.inception_2(Z)
			Z, bottleneck3 = self.inception_3(Z)
		if self.use_residual:
			Z = Z + self.residual(X)
			Z = self.activation(Z)
		if self.return_indices:
			return Z, [i1, i2, i3], self.conv_from_bottleneck_decoder(bottleneck1)
		else:
			return Z#, self.conv_from_bottleneck_decoder(bottleneck1)
