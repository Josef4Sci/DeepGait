...
class VRAE(nn.Module):
    ...
    def __init__(self, sequence_length, number_of_features, hidden_size=90, hidden_layer_depth=2, latent_length=20,
                 ...
                 cuda=False, print_every=100, clip=True, kl_loss_multip=1, max_grad_norm=5, dload='.'):
        ...
        self.logger = TensorboardLogger("C:/Users/josef/Desktop/DeepLearning/Logs/Log5/")
        ...
        if optimizer == 'Adam':
            ...
            self.optimizer = optim.SGD(self.parameters(), lr=learning_rate, momentum=0.995)
        else:
    ...
    def forward(self, x):
        ...
        x_decoded = self.decoder(self.zerovect * latent)
    ...
    def _rec(self, x_decoded, x, loss_fn):
        ...
        kl_loss = -0.5 * self.kl_loss_multip * torch.mean(1 + latent_logvar - latent_mean.pow(2) - latent_logvar.exp())
    ...
    def setNonZeroVect(self, num_ones):
        self.latentNonZeroed=num_ones
        ones=torch.ones(self.batch_size, self.latentNonZeroed)
        zeros = torch.zeros(self.batch_size, 200-self.latentNonZeroed)
        device = torch.device("cuda:0")
        self.zerovect = torch.cat((ones, zeros), 1).to(device)
    ...
    def fit(self, dataset, save = False, name = 'model.pth'):
        """
        ...
        for i in range(self.n_epochs):
            ...
            self.logger.epoch = i
            self.logger.step(1)
            self.logger.log_value('{} Loss'.format("train"), loss)
            if loss < lossMin:
                lossMin = loss
                if save:
                    self.save(name)
